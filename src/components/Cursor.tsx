...
/**
 * Custom desktop cursor (design 7.4): 8px cobalt ink dot + 36px stroked ring, lerp 0.15.
  ...
  useEffect(() => {
    ...
    const onMove = (e: MouseEvent) => {
      ...
      if (dotRef.current) {
        ...
        dotRef.current.style.backgroundColor = hovering ? 'var(--seal)' : 'var(--cobalt)'
      }
    }
  ...
  return (
    <>
      <div
        ref={dotRef}
        aria-hidden="true"
        className="pointer-events-none fixed left-0 top-0 z-[95] hidden h-2 w-2 rounded-full bg-cobalt md:block"
        style={{ transition: 'background-color 0.3s' }}
      />
      <div
        ref={ringRef}
        aria-hidden="true"
        className="pointer-events-none fixed left-0 top-0 z-[95] hidden h-9 w-9 rounded-full border border-cobalt md:block"
